...
```
rules/          stack-agnostic rules — communication, principles, review, handoff
...
presets/        drop-in .claude/ bundles per stack
AGENTS.md       the rules in the cross-tool format (Cursor, Codex, Amp, Zed, Copilot…)
install.sh      wire this repo into ~/.claude
use.sh          drop a preset into a project
```

**Vendored vs referenced.** Only what I authored lives here in full. The 13 third-party
...
- Rules are stack-agnostic; stack standards live in skills. Never state the same rule twice.
