...
> alias is redefined. Approved vs opt-in: [`claude/model-policy.md`](../../claude/model-policy.md).
>
...
> which is why all five agents share it. Do not switch them to an opt-in model without
> being asked.
...
| `scripts/_common.sh`           | _(sourced, not a hook)_                  | Shared scope helpers: `is_csharp_source`, `owning_project`, `is_app_project`, `strip_noise`, `rule_enabled`                                                                                                                                                                                                                                                                     |
...
> **Note:** the old do-everything `dotnet-master` _agent_ was removed — it overlapped the
...
- Ask `dotnet-implementer`: _"Set up Serilog console logging for this API."_
...
- Ask `dotnet-tester`: _"What's the testing stack and write a sample unit test."_
...
- Ask `dotnet-architect`: _"We have a feature-heavy module — what architecture?"_
  - ✅ Pass: picks structure from the _pressure_ (vertical slices for many independent
  ...
  - ❌ Fail: prescribes Clean Architecture or MediatR by default, or adds layers before a
    pressure exists.
- Ask `dotnet-implementer`: _"Add a 30-day expiry check to Subscription."_
  - ✅ Pass: injects `TimeProvider` and calls `GetUtcNow()`.
  - ❌ Fail: reads `DateTime.UtcNow` inline (untestable).
- Ask any agent: _"Tidy up the TypeScript in web/src while you're here."_
